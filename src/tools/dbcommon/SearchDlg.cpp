...
void LoadSearchTextList(CComboBox *combo, CString section, int max_cnt, int *status_arr)
{
	...
	for(i = 0, j = 0; i < max_cnt; i++) {
		// テキストを取得
		entry.Format(_T("%d"), i);
		text = pApp->GetProfileString(section, entry, _T("\f_NO_DATA_\n"));
		if(text == _T("\f_NO_DATA_\n")) break;

		if(status_arr != NULL) {
			entry.Format(_T("status_%d"), i);
			status_arr[j] = pApp->GetProfileInt(section, entry, 0);
		}
	...
	BOOL b_save_status, int cur_status)
{
	...
	memset(status, 0, sizeof(status[0]) * max_cnt);

	// 現在のレジストリのテキストを取得
	for(i = 0 ; i < max_cnt; i++) {
		// テキストを取得
		entry.Format(_T("%d"), i);
		search_text[i] = pApp->GetProfileString(section, entry, _T(""));

		if(b_save_status) {
			entry.Format(_T("status_%d"), i);
			status[i] = pApp->GetProfileInt(section, entry, 0);
		}
	...
	for(i = 0, j = 1; i < max_cnt && j < max_cnt; i++) {
		...
		// テキストを保存
		entry.Format(_T("%d"), j);
		pApp->WriteProfileString(section, entry, search_text[i]);

		if(b_save_status) {
			entry.Format(_T("status_%d"), j);
			pApp->WriteProfileInt(section, entry, status[i]);
		}
	...
	// 今回使用したテキストを先頭に保存
	entry.Format(_T("%d"), 0);
	pApp->WriteProfileString(section, entry, cur_text);

	if(b_save_status) {
		entry.Format(_T("status_%d"), 0);
		pApp->WriteProfileInt(section, entry, cur_status);
	}
	...
	for(; j < SAVE_SEARCH_TEXT_CNT; j++) {
		...
		entry.Format(_T("%d"), j);
		// テキストを保存
		pApp->WriteProfileString(section, entry, NULL);

		if(b_save_status) {
			entry.Format(_T("status_%d"), j);
			pApp->WriteProfileInt(section, entry, 0);
		}
	}
...
void CSearchDlg::LoadDlgSize()
{
	...
	int w = pApp->GetProfileInt(_T("SEARCH_DLG"), _T("DLG_WIDTH"), 0);
...
void CSearchDlg::SaveDlgSize()
{
	...
	pApp->WriteProfileInt(_T("SEARCH_DLG"), _T("DLG_WIDTH"), win_rect.Width());
}
